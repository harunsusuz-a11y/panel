    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'var(--bg)'}}>
      <div style={{background:'var(--s1)',border:'1px solid var(--border)',borderRadius:12,padding:'40px 36px',width:380}}>
        <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:32}}>
          ...
          <div>
            <div style={{fontSize:15,fontWeight:600,color:'var(--text)'}}>Agency ERP</div>
            <div style={{fontSize:10,color:'var(--t3)'}}>Ajans Yönetim Sistemi</div>
          </div>
        </div>
        <h1 style={{fontSize:20,fontWeight:600,marginBottom:6,color:'var(--text)'}}>Giriş Yap</h1>
        ...
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14}}>
          ...
          <button type="submit" disabled={loading} style={{
            background:'var(--gold)',color:'#000',fontWeight:700,fontSize:13,
