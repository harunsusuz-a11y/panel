    ...
    sb.auth.getUser().then(({ data: { user } }) => {
      ...
      channelRef.current = sb.channel(`notif-${user.id}`)
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `user_id=eq.${user.id}` },
          () => {
            // Sadece zil/liste güncellensin. Gerçek OS bildirimi zaten web-push ile
            // service worker'a düşüyor (push/send çağrıldıysa) — burada ayrıca
            // showNotification çağırmak aynı olay için ÇİFT bildirime yol açıyordu.
            load(user.id)
          })
        .subscribe()
    })
